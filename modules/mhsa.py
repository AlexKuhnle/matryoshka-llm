...
class MHSA(torch.nn.Module):
    ...
    def __init__(
        ...
        assert head_size is not None or self.output_size % self.num_heads == 0
        self.head_size = self.output_size // self.num_heads if head_size is None else head_size
        ...
        self.query_proj = torch.nn.Linear(
            self.input_size, self.num_heads * self.qk_size, bias=bias,
        )
        self.key_proj = torch.nn.Linear(
            self.input_size, self.kv_groups * self.qk_size, bias=bias,
        )
        self.value_proj = torch.nn.Linear(
            self.input_size, self.kv_groups * self.head_size, bias=bias,
        )
        self.output_proj = torch.nn.Linear(
            self.num_heads * self.head_size, self.output_size, bias=bias,
        )
    ...
    def forward(
        ...
        query = query.reshape(batch_size, q_length, self.num_heads, self.qk_size) \
            .transpose(-3, -2)
        key = key.reshape(batch_size, q_length, self.kv_groups, self.qk_size) \
            .transpose(-3, -2)
        value = value.reshape(batch_size, q_length, self.kv_groups, self.head_size) \
            .transpose(-3, -2)
